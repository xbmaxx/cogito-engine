...
* 工具洞察语义化：不再罗列工具名和次数，改为行为画像 + 模式检测 + 决策指引。LLM 看到的是"探索型，skill_manage→patch 降级已稳定 3 次"，而不是"terminal: 25 次, 92%"。
* 叙事记忆自动清理：install.py --clean-narratives 用 jieba 从 summary 重新提取焦点词，替换历史 ngram 碎片。用户升级时一键执行，新用户从 1.6.2 起步无污染。
* 结晶引擎加垃圾过滤：detect_candidates 自动跳过含中文标点的 ngram 碎片（如"已重启，"），防止 0.35 置信度的垃圾技能污染结晶库。
* 中文标点碎片拦截：_is_garbage_keyword 新增中文标点符号检测，从源头阻止 "启，看下" "，然后" 等碎片进入焦点栈。
* 结晶门槛放宽到 min_alpha=0.4 / min_count=2，冷启动更快产出第一条技能。
* 工具模式接入结晶引擎：detect_tool_patterns() 从 tool_trace 检测高失败率工具/稳定降级链/全局异常，转化为 CrystallizationCandidate 持久化为跨会话技能。skill_manage 42% 失败 → 结晶为「先 skills_list 确认再调用」。
* keywords.py 启动时一次性 WARNING：告诉用户 jieba 缺失会影响焦点词质量及修复方法。

**问题修复**

* 修复 end_session 重复写入：同一 session 内 narration 不再重复 5-17 次，叙事记录提升 10 倍质量。
* 修复 jieba 缺失：install.py 自动将 jieba 装到 Hermes 的正确 Python 路径，不再降级为字符碎片。

## v1.6.1 — 2026.07.27

**优化改进**

* α 评分持久化：每轮计算的 α 评分和关键信号因子现在写入叙事记忆，技能结晶引擎终于有真实数据可用了。
* 决策指导闭环：会话结束时 LLM 反射被注入 α 评分上下文，能自动提炼用户的偏好和应回避的做法。
* 工具洞察默认开启：引擎现在能看到自己的工具成功率和异常趋势，出错时还能收到具体建议。

**问题修复**

* 修复 α 评分只算不存的问题——算完就丢，下游全部饿死。
* 修复 `include_tool_insights` 默认关闭，导致 LLM 完全感知不到工具调用状况。
* 修复叙事条目缺少 `guidance` 字段，导致 session-end 反射无法向后续会话传递决策指导。

## v1.6.0 — 2026.07.26

**新增功能**

* 记忆重要性区分：引擎能判断哪条记忆更重要，重要的事记得更牢，琐碎的对话自动降权。
* 工具链回溯：引擎记住自己调了多少次工具、成功率如何，出错时能自己总结"最常见错误是什么"。
* 话题模式感知：发现你反复从 A 话题聊到 B 话题时，引擎会注意到这个固定模式。
* 技能结晶：高价值重复模式自动固化为可复用技能，跨会话记住你的偏好和习惯。
* 外部数据安全沙箱：外部记忆库的数据在注入前被标记为未验证，防止被恶意内容操控。
* 决策偏好学习：引擎从历史对话中学习你偏向什么做法、回避什么做法，主动给出更对胃的建议。

**问题修复**

* 修复插件版本声明滞后，导致网关重启后部分 hook 未正确注册。
* 修复部分记忆数据散落两处路径，重启后统一归位。

## v1.5.5 — 2026.07.19

**优化改进**

* 新增外部知识库接口语义检索：fastembed + BAAI/bge-small-zh-v1.5 本地嵌入。
* 持久化文件扩容，最长可追踪近 30 天的情绪变化与叙事记忆。
* 新增时间范围查询 + 聚合统计，可回溯最近 N 天的情绪与叙事变化。
* 新增情绪趋势按自然语言摘要。
* 新增跨会话模式检测、关键词共现聚类。
...
* 外部知识库接入层：让用户的任何外部记忆库（Fact Store / Obsidian / Logseq / 自定义），通过引擎接口，自动注入到每轮对话的 XML 中，实现记忆增强。

* 深度信息加工：把 XML 的信息从呈现变成触发指令，对情绪、叙事记忆、外部知识库信息做智能处理，为 LLM 提供高价值信息。

## v1.5.2 — 2026.07.11

**新增功能**

* 情绪自主反思：心跳模式切换时，引擎用 LLM 生成 1-2 句主观感受文本，自然融入对话上下文，不出现 AI 报告腔。
* 主动续接提示：新会话自动感知上次未聊完的话题和未解问题，以内在状态方式静默传递，等话题自然拐到才开口。
* 人味翻译：XML 输出语言全面去 AI 化。

**优化改进**

* 天气缓存：首条消息触发天气查询，后续仅在用户提及天气/位置时按需刷新，减少网络开销。
* 定位策略更加智能，即使开了代理，也不会把地理位置漂移到海外，确保和本地 IP 位置一致，可联动查询本地天气情况。
* reflection LLM 支持 streaming SSE 降级：非流式 API 返回 400 时自动切流式解析，适配 Copilot / WorkBuddy 等仅流式 provider。
* 心跳快照持久化路径统一至 memory 目录，与其他引擎数据保持一致。
