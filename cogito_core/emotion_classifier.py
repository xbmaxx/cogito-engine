...
class EmotionClassifier:
    ...
    def classify(self, text: str) -> Dict[str, Any]:
        ...
        # 英文技术词汇交叉验证（从 engine.py 迁入，内化 DUTIR 特定逻辑）
        if dominant != "none" and _has_ascii_word(text):
            from .text_emotion import quick_sentiment
            qs = quick_sentiment(text)
            if qs.get("confidence", 0) == 0.0:
                # 快速检测无情绪信号 → 英文技术/代码文本，归为中性
                return {
                    "available": True,
                    "emotions": {k: 0.0 for k in self.MODEL_DIMS},
                    "dominant": "none",
                    "confidence": 0.0,
                    "method": "dutir_cross_validated",
                }
